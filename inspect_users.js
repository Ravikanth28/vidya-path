...
async function main() {
    ...
    if (!dbUrl) {
        throw new Error('No database URL found in server/.env');
    }
    ...
    try {
        const [rows] = await conn.query(
            `SELECT id, name, email, role, status, password
             ...
             ORDER BY FIELD(role, 'admin', 'student'), created_at ASC, id ASC`
        );
        ...
        const sanitized = rows.map((row) => ({
            ...
            role: row.role,
            status: row.status,
            passwordType: row.password && (row.password.startsWith('$2a$') || row.password.startsWith('$2b$')) ? 'bcrypt' : 'plaintext',
            passwordPreview: row.password && (row.password.startsWith('$2a$') || row.password.startsWith('$2b$')) ? row.password.slice(0, 12) + '...' : row.password
        }));
